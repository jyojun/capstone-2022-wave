...
function PlaceRepleContent(props) {
  ...
  const SubmitHandler = (e) => {
    ...
    let body = {
      uid: user.uid,
      reple: Reple,
      placeId: props.Reple.placeId,
      repleId: props.Reple._id,
    };

    axios.post("/api/placeReple/edit", body).then((res) => {
      if (res.data.success) {
  ...
  const DeleteHandler = (e) => {
    ...
    if (window.confirm("정말로 삭제하시겠습니까?")) {
      ...
      axios
        .post("/api/placeReple/delete", body)
        .then((res) => {
          if (res.data.success) {
            alert("댓글이 삭제되었습니다.");
...
export default PlaceRepleContent;
